...
import (
	...
	vtemplate "github.com/shenjing023/vivy-polaris/cmd/template"
)
...
var (
	...
	//go:embed template/config/config.yaml
	configYamlTemplate []byte
)

func main() {
	// go mod init
	cmd := exec.Command("go", "mod", "init", Cfg.ModName)
	_, err := RunExecCommand(cmd, false, 3)
	if err != nil {
		panic(err)
	}
	// mkdir config
	...
	os.WriteFile("config/config.yaml", configYamlTemplate, os.ModePerm)
